#!/usr/bin/env bash
# TVPO | MoE | vLLM rollout | Megatron training | NVIDIA GPUs
# TVPO puts the total-variation trust region on the batch (or the prompt group) instead of the
# individual token: while the realised divergence is inside the region every token is updated, and
# once it leaves the region only the tokens whose update pulls the policy back towards pi_old
# survive. It is a variant of DPPO-Binary-TV (paper: arXiv:2602.04879).
...
# TVPO reads `clip_ratio` as the budget on the L1 distance sum_a |pi(a|s) - pi_old(a|s)|;
# the TV threshold it compares the divergence estimate against is half of it.
# `clip_ratio_low` / `clip_ratio_high` are unused by this loss.
clip_ratio=${CLIP_RATIO:-0.015}

# Scope the divergence estimate is taken over: prompt (per prompt group) or batch.
tv_scope=${TV_SCOPE:-prompt}

# Disable dual-clip PPO and TIS, as the DPPO reference scripts do.
...
project_name=${PROJECT_NAME:-verl_tvpo_qwen3_moe}
experiment_name=${EXPERIMENT_NAME:-qwen3_30b_a3b_tvpo_${tv_scope}}
# ---- end user-adjustable ----
...
DATA=(
    ...
    # Dr. GRPO: drop the difficulty bias, and pair it with seq-mean-token-sum-norm below
    # to drop the length bias. See https://arxiv.org/pdf/2503.20783.
...
ACTOR=(
    ...
    actor_rollout_ref.actor.policy_loss.tvpo_tv_scope=${tv_scope}
